...
det_val = np.linalg.det(matrix_weight)

if abs(det_val) < 1e-12:
    print("Ma trận weight không khả nghịch (định thức ≈ 0).")
else:
    print("Định thức của ma trận weight =", det_val)
    inv_matrix_weight = np.linalg.inv(matrix_weight)
    print("Ma trận nghịch đảo (weight^-1) là:")
    print(inv_matrix_weight)
